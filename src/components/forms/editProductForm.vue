...
import { compareRecursive } from '@/util/helpers';
...
export default {
	...
	methods:{
		cloneRecord(){
			this.item = this.record ? {...this.record} : productBuilder();
		},
		async doCreate(){
			this.saving = true;
			let created = await this.server.put(`/api/product/${this.item.id}`, this.item);
	...
	computed: {
		hasChanges(){
			return compareRecursive(this.item, this.record);
		}
	},
	created(){
		this.cloneRecord();
